...
class DataLoader3DWeighted(DataLoader3D):
    def __init__(self, *args, **kwargs):
        super(DataLoader3DWeighted, self).__init__(*args, **kwargs)
        self.weightmap_shape = self.determine_weightmap_shape()

    def determine_weightmap_shape(self):
        weightmap_shape = (self.batch_size, 1, *self.patch_size)
        return weightmap_shape

    def generate_train_batch(self):
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        weightmap = np.zeros(self.weightmap_shape, dtype=np.float32)
        case_properties = []
        for j, i in enumerate(selected_keys):
            ...
            weightmap_fname = self._data[i]['data_file'][:-4] + "_weightmap.npy"
            if isfile(weightmap_fname):
                weightmap_data = np.load(weightmap_fname, self.memmap_mode)
            else:
                raise FileNotFoundError(f"Could not find weightmap file: {weightmap_fname}")
            ...
            weightmap_data = np.copy(weightmap_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
                                    valid_bbox_y_lb:valid_bbox_y_ub,
                                    valid_bbox_z_lb:valid_bbox_z_ub])
            ...
            weightmap[j, 0] = np.pad(weightmap_data, ((0, 0),
                                                    (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                    (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                    (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                               'constant', **{'constant_values': 0})

        return {'data': data, 'seg': seg, 'weightmap': weightmap, 'properties': case_properties, 'keys': selected_keys}
